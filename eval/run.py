...
使い方:
    ...
    python eval/run.py --tag win --compare-mac  # Mac 版の実測値と並べる

Mac 版 (MLX) との違いは乱数だけ。サンプリング条件 (temperature 0.8 /
top_k 40 / repetition_penalty 1.15 / seed 777) は同じにしてあるが、
乱数生成器が別物なので同じ文章は出ない。bits/char は乱数を使わないので
そのまま比較できるが、返答から測る3指標 (反復率・主題保持率・破綻率) は
標本のばらつきを含む。20問しかないので 1問の当たり外れで 0.05 動く。
"""
...
# Mac 版 (BASELINES.md 3.2 の「最終」列) の実測値。
MAC_FINAL = {
    "bits_per_char": 2.584,
    "topic_rate": 0.733,
...
def bits_per_char(model, tokenizer: Tokenizer, text: str, batch_size: int = 16) -> float:
    ...
    autocast は使わない。bf16 は仮数が7bitしかなく、合計で 3桁目が揺れる。
    採点は比較のための数値なので、速さより再現性を取る。
    """
    ...
    for start in range(0, len(windows), batch_size):
        chunk = np.array(windows[start : start + batch_size], dtype=np.int64)
...
def show(summary: dict, baseline: dict | None) -> None:
    def line(label: str, key: str, fmt: str = "{:.3f}", better: str = "low") -> str:
        ...
        if baseline and baseline.get(key) is not None:
            before = baseline[key]
            label_before = baseline.get("tag", "前回")
            delta = now - before
            arrow = "改善" if (delta < 0) == (better == "low") else "悪化"
            if abs(delta) < 1e-9:
                arrow = "変化なし"
            text += f"   ({label_before} {fmt.format(before)} / {delta:+.3f} {arrow})"
        return text
...
def main() -> None:
    ...
    ap.add_argument(
        "--compare-mac",
        action="store_true",
        help="Mac 版 (BASELINES.md 3.2 の最終列) と並べる",
    )
    ...
    if not HOLDOUT.exists():
        raise SystemExit("固定検証セットがありません。先に --make-holdout を実行してください。")
    ...
    elif args.compare_mac:
        baseline = MAC_FINAL
    show(summary, baseline)
    if args.compare_mac:
        print()
        print("  bits/char は乱数を使わないので直接比べられる。")
        print("  返答から測る3指標は、乱数生成器が違うぶん標本のばらつきを含む。")
