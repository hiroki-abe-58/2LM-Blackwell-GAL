...
class Curve:
    ...
    # 経過秒。Mac 版と速度を比べる図で横軸に使う。
    elapsed: list[tuple[int, float]] = field(default_factory=list)

    @property
    def elapsed_by_step(self) -> dict[int, float]:
    ...
    def total_sec(self) -> float:
        ...
        train.py が最後に出す合計時間はサンプル生成を含むので少し大きい。
        図と本文で数字を揃えるため、出典はどちらも loss.csv にする。
        """
        return self.elapsed[-1][1] if self.elapsed else float("nan")
    ...
    with open(path, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("elapsed_sec"):
                elapsed.append((step, float(row["elapsed_sec"])))
    return Curve(
        name or path.stem,
        keep_last_run(train),
        keep_last_run(val),
        keep_last_run(elapsed),
    )
