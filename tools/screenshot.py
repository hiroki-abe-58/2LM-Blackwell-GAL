...
コンソールエラーと未捕捉例外も拾うので、GUI の回帰確認としても使える。
教材本体には不要なユーティリティ。

    python server.py                          # 別のターミナルで起動しておく
    python tools/screenshot.py --out docs/images

Windows では Chrome が入っていない環境もあるので、無ければ Edge を使う。
Edge は Chromium なので DevTools Protocol はそのまま通る。
"""
...
PROFILE = Path(tempfile.gettempdir()) / "2lm-chrome-profile"


def find_browser() -> str:
    """Chrome を優先し、無ければ Edge を使う.

    os.environ は Windows ではキーを大文字化して持つので、
    ** で展開すると 'ProgramFiles' では引けなくなる。個別に get する。
    """
    for var, relative in BROWSER_CANDIDATES:
        ...
        if path.exists():
            return str(path)
    raise SystemExit("Chrome も Edge も見つかりませんでした")
...
def launch_browser(port: int) -> subprocess.Popen:
    ...
    for _ in range(60):
        try:
            ...
            time.sleep(0.5)
    raise SystemExit("ブラウザの起動に失敗しました")
