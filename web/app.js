...
async function loadInfo() {
  try {
    ...
    state.info = info;
    // 文脈長の単位はトークン。サブワードでは1トークンが平均2.4文字ほど運ぶので、
    // 「文字」と書くと文脈の長さを実際より短く見せてしまう。
    const unit = info.tokenizer === "subword" ? "トークン" : "文字";
    el.meta.textContent =
      `${info.params_m}M params / 語彙 ${info.vocab_size} / 文脈 ${info.block_size}${unit}`;
    el.spec.textContent = [
      `checkpoint : ${info.checkpoint}`,
      `tokenizer  : ${info.tokenizer === "subword" ? "サブワード (SentencePiece)" : "文字レベル"}`,
      `layers     : ${info.n_layer}`,
      `heads      : ${info.n_head}`,
      `d_model    : ${info.n_embd}`,
